...
const UKDocumentVerification: React.FC<StepComponentProps> = ({
  ...
  const [ninoCaptured, setNINOCaptured] = useState(!!formData.nino);
  
  const primaryColor = '#1565c0'; // Royal blue for UK
  ...
  // NINO Hook (National Insurance Number)
  const ninoHook = useDocumentCapture({
    ...
    onDataExtracted: (extractedData) => {
      if (extractedData.idNumber || extractedData.panNumber) {
        updateFormData({ ninoNumber: extractedData.idNumber || extractedData.panNumber });
        setNINOCaptured(true);
      }
    },
  });
  ...
  return (
    <Box>
      <Typography variant="h5" sx={{ fontWeight: 700, mb: 4, color: '#333' }}>
        Document Verification - United Kingdom
      </Typography>

      <Grid container spacing={3}>
        {/* Live Photo Card */}
        <Grid item xs={12} sm={6} md={4}>
          <Paper elevation={2} sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', borderRadius: 2, border: '1px solid #e0e0e0' }}>
            <Box sx={{ width: 60, height: 60, borderRadius: '50%', bgcolor: '#e3f2fd', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
        ...
        <Grid item xs={12} sm={6} md={4}>
          <Paper elevation={2} sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', borderRadius: 2, border: '1px solid #e0e0e0' }}>
            ...
            <Typography variant="body2" sx={{ color: '#666', mb: 3, minHeight: 48 }}>
              Show your UK Passport or Driving Licence to the camera
            </Typography>
            <Button
              variant="outlined"
              startIcon={<PhotoCameraIcon />}
        ...
        <Grid item xs={12} sm={6} md={4}>
          <Paper elevation={2} sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', borderRadius: 2, border: '1px solid #e0e0e0' }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>National Insurance Number</Typography>
            <Typography variant="body2" sx={{ color: '#666', mb: 3, minHeight: 48 }}>
              Show your NINO letter or card. Your NI number format: AB 12 34 56 C
            </Typography>
            <Button
              variant="outlined"
              startIcon={<PhotoCameraIcon />}
              onClick={handleNINOStart}
              disabled={ninoCaptured || !govIdCaptured}
              sx={{ borderColor: primaryColor, color: primaryColor }}
            >
              {ninoCaptured ? 'NINO Captured ✓' : 'Capture NINO'}
            </Button>
            {ninoCaptured && <CheckCircleIcon sx={{ color: successColor, mt: 1 }} />}
          </Paper>
        </Grid>
      </Grid>

      {/* Camera Dialogs - Same structure as US */}
      <Dialog open={livePhotoHook.showCamera} onClose={livePhotoHook.stopCamera} maxWidth="md" fullWidth>
        <DialogTitle>
          Liveness Verification
          <IconButton onClick={livePhotoHook.stopCamera} sx={{ position: 'absolute', right: 8, top: 8 }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
      ...
      <Dialog open={govIdHook.showCamera} onClose={govIdHook.stopCamera} maxWidth="md" fullWidth>
        <DialogTitle>
          Government ID Verification - UK
          <IconButton onClick={govIdHook.stopCamera} sx={{ position: 'absolute', right: 8, top: 8 }}><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent>
          <Alert severity="info" sx={{ mb: 2 }}>Show your <strong>UK Passport or Driving Licence</strong> to the camera.</Alert>
          ...
          <Box sx={{ textAlign: 'center', mt: 2 }}>
            <Button variant="contained" onClick={govIdHook.capturePhoto} disabled={govIdHook.processing} sx={{ bgcolor: primaryColor }}>
              {govIdHook.processing ? 'Processing...' : 'Capture ID'}
            </Button>
          </Box>
        </DialogContent>
      </Dialog>

      <Dialog open={ninoHook.showCamera} onClose={ninoHook.stopCamera} maxWidth="md" fullWidth>
        <DialogTitle>
          National Insurance Number Verification - UK
          <IconButton onClick={ninoHook.stopCamera} sx={{ position: 'absolute', right: 8, top: 8 }}><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent>
          <Alert severity="info" sx={{ mb: 2 }}>Show your <strong>National Insurance Number</strong> letter or card. Format: AB 12 34 56 C</Alert>
          <Box sx={{ position: 'relative', width: '100%', maxWidth: 600, height: 400, mx: 'auto', border: `2px solid ${primaryColor}`, borderRadius: 2, overflow: 'hidden' }}>
            <video ref={ninoHook.videoRef} autoPlay playsInline muted style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            <canvas ref={ninoHook.canvasRef} style={{ display: 'none' }} />
            {ninoHook.processing && (
              <Box sx={{ position: 'absolute', inset: 0, bgcolor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <CircularProgress sx={{ color: primaryColor }} />
              </Box>
          ...
          <Box sx={{ textAlign: 'center', mt: 2 }}>
            <Button variant="contained" onClick={ninoHook.capturePhoto} disabled={ninoHook.processing} sx={{ bgcolor: primaryColor }}>
              {ninoHook.processing ? 'Processing...' : 'Capture NINO'}
            </Button>
          </Box>
        </DialogContent>
      </Dialog>
...
export default UKDocumentVerification;
